...
const parseFormulaOrNumber = (value) => {
...
const applyRules = (rules, query) => {
  return _.reduce(rules, (accum, rule) => {
    ...
    if(!!query.vars) {
      _.forEach(query.vars, (v, k) => parser.setVariable(k, parseFormulaOrNumber(v).result));
    }

    if(!!rule.vars) {
      _.forEach(rule.vars, (v, k) => parser.setVariable(k, parseFormulaOrNumber(v).result));
    }

    if(!!rule.rate) {
      tax = {...tax, rate: parseFormulaOrNumber(rule.rate)};
    ...
    if(!!rule.amount) {
      tax = {...tax, amount: parseFormulaOrNumber(rule.amount)};
    }
